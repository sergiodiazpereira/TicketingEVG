...
### H. Restricciones de Trabajadores, Visualización de Responsables y Filtros de Prioridad (Revisión Final - 31-05-2026)
* **Restricción de Edición para Trabajadores:** Se ha inhabilitado la edición de la información base de los tickets para roles de `'trabajador'` u `'operario'`, tanto en el frontend (getter `puedeEditar` en `ModalTicketComponent`) como en el controlador del backend (`C_Ticket::actualizar`). Esto garantiza que los técnicos no puedan modificar la descripción, asunto ni categoría originales reportados por los usuarios.
* **Dualidad de Roles en Trabajadores Creadores:** Si un trabajador crea un ticket de soporte, se implementó el getter `debeVerComoProfesor` en frontend y el helper `debe_ver_como_profesor` en el backend. Si el trabajador es el creador del ticket y no está asignado a sí mismo como técnico, la aplicación lo trata exactamente como a un solicitante estándar (`profesor`), bloqueando las acciones técnicas (Iniciar Proceso, Marcar Resuelto) y permitiéndole editar o cancelar el ticket antes de que sea procesado. Esto se anula si un Administrador/Responsable lo asigna como encargado del mismo.
* **Filtro Dinámico de Prioridad:** Se agregó un nuevo filtro desplegable personalizado por prioridad (`Alta`, `Media`, `Baja`) en el listado general de tickets, operando con la misma estructura reactiva y visual que los filtros preexistentes.
* **Reorganización Estructurada de Buscador y Filtros:** Se rediseñó la maquetación en `lista-tickets.component.html` y `.css` implementando un contenedor de filtros `.filtros-contenedor` posicionado en una fila inferior debajo de la barra de búsqueda general, resolviendo descuadres visuales y mejorando la adaptabilidad responsiva.
* **Trazabilidad Extendiva para Responsables:** Se actualizó `M_Ticket::listar_por_usuario` en el backend para que los usuarios con el rol `'responsable'` (supervisores de área) puedan visualizar todos los tickets pertenecientes a las categorías en las que están asignados, cerrando la brecha de visibilidad y permitiendo una correcta asignación y reasignación técnica de tickets sin asignar.
* **Depuración de Scripts Temporales:** Se eliminaron permanentemente todos los scripts obsoletos de refactorización escritos en Python (`.py`) en el directorio raíz del proyecto, manteniendo el repositorio limpio y enfocado exclusivamente en las tecnologías de producción (PHP y Angular).
